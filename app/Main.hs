...
data SGAction = North | East | South | West deriving (Eq, Ord)
...
action s a | s == (0,0) = s
  ...
  where
    applyAction c m = if (0 <= xx) && (xx <= 3) && (0 <= yy) && (yy <= 3)
      then (xx, yy)
      else c
      where xx = fst c + fst m
            yy = snd c + snd m

sgTransition :: SGAction -> SGState -> SGState -> Double
sgTransition a s s' | s' == (0,0) || s' == (3,3) = 0.0
                    | s' == action s a = 1.0
                    | otherwise = 0.0
...
sgMDP = MDP { states = S.fromList [(x, y) | x <- [0..3], y <- [0..3]]
...
main = do
  let sgmdp = sgMDP
  mapM_ (\s -> print (s, policyEvaluation sgmdp sgPolicy 3 s)) (S.toList (states sgmdp))
  mapM_ (\s -> print (s, policyEvaluationM sgmdp sgPolicy 3 s)) (S.toList (states sgmdp))
